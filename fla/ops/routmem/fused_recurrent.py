...
@triton.jit
def fused_recurrent_routmem_inference_kernel(
    q,
    k,
    v,
    s,
...
def fused_recurrent_routmem_inference(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    s: torch.Tensor,
    ...
    grid = (B * HQ,)
    fused_recurrent_routmem_inference_kernel[grid](
        q,
        k,
        v,
        s,
...
def fused_recurrent_routmem_fwd(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    s: torch.Tensor,
...
def fused_recurrent_routmem_bwd(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    s: torch.Tensor,
...
class FusedRecurrentRoutMemFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        if T == 1 and not q.requires_grad:
            o, (hkt, hvt) = fused_recurrent_routmem_inference(
                q=q,
                k=k,
                v=v,
                s=s,
            ...
            return o, hkt, hvt
        ok, hkt, qv, ov, hvt = fused_recurrent_routmem_fwd(
            q=q,
            k=k,
            v=v,
            s=s,
    ...
    def backward(ctx, do, dhkt=None, dhvt=None):
        ...
        dq, dk, dv, ds, dg, dhk0, dhv0 = fused_recurrent_routmem_bwd(
            q=q,
            k=k,
            v=v,
            s=s,
...
def fused_recurrent_routmem(
    ...
    o, *final_state = FusedRecurrentRoutMemFunction.apply(
        q,
        k,
        v,
        s,
